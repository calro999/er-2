...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }): Promise<Metadata> {
  ...
  try {
    ...
    return {
      ...
      alternates: {
        canonical: `https://er-2.pages.dev/posts/${id}`,
      },
      openGraph: {
        title: `${post.title} 【品番：${hinbanText}】`,
        description: descriptionText,
        url: `https://er-2.pages.dev/posts/${id}`,
        type: "article",
...
export default async function PostPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    "itemListElement": [
      {
        ...
        "position": 1,
        "name": "ホーム",
        "item": "https://er-2.pages.dev"
      },
      {
        "@type": "ListItem",
        "position": 2,
        "name": post.title,
        "item": `https://er-2.pages.dev/posts/${post.id}`
      }
    ]
  };
  ...
  const articleSchema = {
    ...
    "author": { "@type": "Organization", "name": "バクロファイル" },
    "publisher": { "@type": "Organization", "name": "バクロファイル" },
      ...
      <article className="space-y-6 max-w-4xl mx-auto" lang="ja">
        ...
        <div className="border border-slate-200 bg-white rounded-2xl p-6 md:p-10 shadow-sm space-y-8">
          ...
          {post.actresses && post.actresses.length > 0 && (
            <section className="pt-6 border-t border-slate-100 space-y-4" aria-label="関連作品・出演女優の作品">
              <h3 className="text-xs font-bold text-slate-500 uppercase tracking-widest">
                ▼ 出演女優の関連作品・動画をチェック！
              </h3>
              <div className="flex flex-col gap-3">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5 pt-2">
                  {post.actresses.map((actress) => {
                    ...
                    const searchUrl = `https://al.fanza.co.jp/?lurl=https%3A%2F%2Fwww.dmm.co.jp%2Fsearch%2F-%2F%3Fsearchstr%3D${encodedActress}%2F&af_id=onchan555-007`;
                    return (
                      <a
                        key={actress}
                        href={searchUrl}
