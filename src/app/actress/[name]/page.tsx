...
export async function generateMetadata({ params }: { params: Promise<{ name: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://er-2.pages.dev/actress/${name}` },
    openGraph: { title: censorText(titleText), description: censorText(descriptionText), url: `https://er-2.pages.dev/actress/${name}`, type: "website", images: ogImage ? [{ url: ogImage, width: 800, height: 538 }] : [] },
...
export default async function ActressPage({ params }: { params: Promise<{ name: string }> }) {
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": `${actressName} レビュー`, "item": `https://er-2.pages.dev/actress/${name}` }
    ]
  };
  const faqSchema = {
