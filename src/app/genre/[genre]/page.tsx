...
export async function generateMetadata({ params }: { params: Promise<{ genre: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    alternates: { canonical: `https://er-2.pages.dev/genre/${genre}` },
    openGraph: { title: censorText(titleText), description: censorText(descriptionText), url: `https://er-2.pages.dev/genre/${genre}`, type: "website", images: ogImage ? [{ url: ogImage, width: 800, height: 538 }] : [] },
...
export default async function GenrePage({ params }: { params: Promise<{ genre: string }> }) {
  ...
  const breadcrumbSchema = {
    ...
    "itemListElement": [
      ...
      { "@type": "ListItem", "position": 2, "name": `${genreName} レビューまとめ`, "item": `https://er-2.pages.dev/genre/${genre}` }
    ]
  };
  const faqSchema = {
