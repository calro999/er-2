...
# er-2: ハーレム・SM・緊縛・凌辱・ハード系漫画
SEARCH_KEYWORDS = ["ハーレム", "SM", "緊縛", "凌辱", "調教", "拘束", "奴隷"]
...
def generate_manga_article(item):
    ...
    prompt = f"""以下のアダルト漫画（ハード系）の情報を元に、SEO完全特化の高品質ブログ記事HTML本文を生成してください。
...
2. 2000文字以上
3. 「{cid}」「レビュー」「感想」「ネタバレなし」「あらすじ」「SM漫画」「ハーレム漫画」「おすすめ」を自然に含める
4. 必須セクション: 概要・設定紹介（ネタバレなし）/見どころ（箇条書き）/作者・画風の分析/こんな人におすすめ/評価表(table: ストーリー・エロ度・画力)/FANZAで読む方法
5. ネタバレは最小限にし「続きはFANZAで確認」へ誘導
6. HTMLのみ出力。マークダウン禁止。"""

    system_message = "あなたはアダルト漫画に精通したカリスマレビュアーです。SM・ハーレム・凌辱系に詳しく、品番検索・タイトル検索で辿り着く読者向けのSEO最強記事をHTMLで書きます。"
